...
from villagers.models import Villager, Importer
...
class PartialSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {'town': {'required': False},
    ...
    def update(self, obj, validated_data):
        obj = obj.villagers.filter(citizen_id=self.context['view'].kwargs['citizen_id']).first()
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            for key, values in relatives.items():
                [relatives[item].append(key) for item in values if key not in relatives[item]]

            for villager in new_villagers:
                if relatives[villager.citizen_id]:
                    villager.relatives.set(
                        vlgs.id for vlgs in new_villagers if vlgs.citizen_id in relatives[villager.citizen_id])
        return current_import

    def to_representation(self, instance):
        response = super(ImportSerializer, self).to_representation(instance)
        response['data'] = {
            'import_id': instance.import_id
        }
        return response
        ...
        for month, month_value in data.items():
            if data[month]:
                ...
                for villager_and_present in set(month_value):
                    ...
                    total_data = {"citizen_id": villager_and_present,
                                  "presents": presents}
