...
from villagers.serializers import (ImportSerializer, PercentileSerializer,
                                   BirthdaySerializer, PartialSerializer, UPCitizenSerializer)
from villagers.models import Villager, Importer
from villagers.filters import VillagersFilter


class CreateVillagerView(CreateModelMixin, RetrieveModelMixin,
    ...
    @action(methods=['get'], detail=False,
            url_path='citizens/(?P<import_id>[^/.]+)', url_name='citizens')
    def citizens(self, request, import_id):
        return self.list(request)

    @action(methods=['get'], detail=True,
            url_path='citizens/birthdays', url_name='citizens-birthdays')
