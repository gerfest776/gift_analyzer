...
class TestApi(APITestCase):
    ...
    def test_create_importer(self):
        ...
        self.assertEqual(
            list(Importer.objects.filter(import_id=2).first().villagers.values_list('citizen_id', flat=True)),
    ...
    def test_patch(self):
        ...
        self.assertEqual(Villager.objects.filter(citizen_id=2).first().name, patch_data["name"])
        self.assertEqual(list(Villager.objects.filter(citizen_id=2).first().relatives.values_list('id', flat=True)),
